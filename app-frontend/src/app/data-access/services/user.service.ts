...
export class UserService {
  ...
  get$ = (id: number) =><Observable<CustomResponse>>this.http.get<CustomResponse>(`${this.apiUrl}/user/${id}`).pipe(
    tap(console.log),
    catchError(this.handleError)
  )
  ...
  login(email: string, password: string): Observable<boolean>{
    ...
    return this.http.post<CustomResponse>(`${this.apiUrl}/user/login`, body).pipe(
      tap(console.log),
  ...
  register(user: User): Observable<boolean>{
    return this.http.post<CustomResponse>(`${this.apiUrl}/user/save`, user).pipe(
      tap(console.log),
      map((response: CustomResponse) =>{
